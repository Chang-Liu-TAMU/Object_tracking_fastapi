#based on https://github.com/GeekAlexis/FastMOT/blob/master/fastmot/videoio.py
...
class VideoIO:
    def __init__(self, size, input_uri=0,
                 output_uri=None,
        ...
        self.protocol = None
        # self.is_live = True # who does this
        self.is_live = False
        if self.input_uri == "0":
            self.input_uri = 0
        ...
        while not self.exit_event.is_set():
            ...
            with self.cond:
                if not ret:
                    self.exit_event.set()
                    self.cond.notify()
                    # print("total frame count from subthread: {}".format(self.frame_count))
                    break
